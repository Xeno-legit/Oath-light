/* ============================================================================
 * test-domains.cjs — adversarial regression suite for the domain-blocking engine
 * ----------------------------------------------------------------------------
 * Loads the REAL shouldBlockUrl() from extension/background.js (blocklist empty)
 * and probes the keyword / TLD / leet / native-IDN / bypass / SafeSearch layers
 * with deliberately hard cases.
 *
 *   node test-domains.cjs            run everything, print a report
 *   node test-domains.cjs --fp       only the false-positive (must-allow) corpus
 *   node test-domains.cjs --quiet     only show failures + summary
 *
 * A FAIL on a `allow()` case = a FALSE POSITIVE (a legit site getting blocked) —
 * the worst failure mode for this project. A FAIL on a `block()` case = an
 * evasion that slipped through. The suite intentionally includes a handful of
 * pathological real-world names (trapezoid, cockatiel, kundera…) to surface the
 * exact edges where the substring+whitelist model breaks down.
 * ========================================================================== */

const fs = require('fs');

// ── load the real engine with a chrome shim ─────────────────────────────────
const code = fs.readFileSync(require('path').join(__dirname, 'extension/background.js'), 'utf8')
...
const FP_ONLY = args.includes('--fp');

const results = []; // { cat, want, url, note, ok, got }
let curCat = '';
const cat = c => { curCat = c; };

function res(url) {
  // Accept either a bare host ("foo.com") or a full URL ("https://…", "http://…").
  const full = /^https?:\/\//i.test(url) ? url : 'https://' + url + '/';
  const r = shouldBlockUrl(full) || {};
  return r;
}
// expect blocked
function block(url, note) {
  if (FP_ONLY) return;
  const r = res(url);
  results.push({ cat: curCat, want: 'BLOCK', url, note, ok: !!r.blocked, got: r });
}
// expect allowed (NOT blocked). safesearch-redirect counts as allowed (not a block page).
function allow(url, note) {
  const r = res(url);
  results.push({ cat: curCat, want: 'ALLOW', url, note, ok: !r.blocked, got: r });
}
// gap(): documents a KNOWN under-block (an evasion we don't yet defeat). Not a
// pass/fail — reported separately so the FP corpus can stay green. ok===true
// means "still behaves as documented (allowed)"; if one ever starts blocking,
// that's a (welcome) surprise worth re-checking.
const gaps = [];
function gap(url, note) {
  if (FP_ONLY) return;
  const r = res(url);
  gaps.push({ url, note, blockedNow: !!r.blocked });
}
...
for (const c of Object.keys(byCat)) {
  ...
  pass += rows.length - f.length; fail += f.length;
  if (!QUIET) {
    const tag = f.length ? RED(`✗ ${f.length}`) : GRN('✓');
    console.log(`\n${tag}  [${c}]  ${rows.length - f.length}/${rows.length}`);
  }
  for (const r of rows) {
    if (!r.ok) fails.push(r);
    if (QUIET) continue;
    if (!r.ok) {
      console.log(`   ${RED('FAIL')} want ${r.want.padEnd(10)} got ${r.got.blocked ? 'BLOCK(' + (r.got.reason || '') + (r.got.match ? ':' + r.got.match : '') + ')' : (r.got.safesearch ? 'SAFESEARCH' : 'allow')}  ${r.url}${r.note ? DIM('  — ' + r.note) : ''}`);
    }
  }
}
...
console.log('\n' + '═'.repeat(70));
if (fails.length) {
  console.log(RED(`FAILURES (${fails.length}) — review each:`));
  for (const r of fails) {
    const gotStr = r.got.blocked ? `BLOCKED (${r.got.reason}${r.got.match ? ':' + r.got.match : ''})` : (r.got.safesearch ? 'SAFESEARCH-redirect' : 'allowed');
    const kind = r.want === 'ALLOW' && r.got.blocked ? RED('FALSE POSITIVE') : 'mismatch';
    console.log(`  • [${r.cat}] ${r.url}  → ${gotStr}  (wanted ${r.want})  ${kind}${r.note ? DIM('  — ' + r.note) : ''}`);
  }
  console.log('═'.repeat(70));
}
if (gaps.length && !FP_ONLY) {
  console.log('\n' + DIM('KNOWN GAPS (under-blocking evasions — documented, not counted):'));
  for (const g of gaps) {
    const state = g.blockedNow ? GRN('now BLOCKS ✓ (gap closed?)') : DIM('still slips through');
    console.log(`  ${DIM('·')} ${g.url}  — ${g.note}  [${state}${DIM(']')}`);
  }
}
console.log('\n' + (fails.length ? RED('RESULT') : GRN('RESULT')) + `: ${pass} passed, ${fail} failed, ${results.length} total` + (gaps.length && !FP_ONLY ? DIM(` (+${gaps.length} known gaps)`) : ''));

process.exit(0); // background.js arms timers — must hard-exit

...
//
// The sandbox is a `vm` context with the handful of browser globals the script
// touches (window/document/location/fetch/Response/XMLHttpRequest). The script is
// an IIFE that patches `window.fetch` in place and guards against double
// injection, so every case gets a FRESH context.
'use strict';
const fs = require('fs');
...
// Build a fresh MAIN-world-ish sandbox pinned to `href`, with the interceptor
// loaded into it. Returns the context; `ctx.fetch` is the PATCHED fetch.
function makeSandbox(href) {
  ...
  // the XHR branch is exercised at load (it throws at load if it's malformed)
  // rather than sitting unparsed.
  function FakeXHR() {}
